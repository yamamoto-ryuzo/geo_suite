...
- セッション再現・監査ログの自然文要約:
	- セッション履歴を MCP で保持し、モデルが「誰が何をしたか」の要約や監査レポートを作成。
- ユーザ・ロールに基づく提案のカスタマイズ:
	- ユーザ役割（管理者／編集者／閲覧者）と権限を文脈に含め、モデル出力を権限に応じて変える。
- 安全ガード（提案→サンドボックス→承認→実行）:
	- モデル生成クエリ・変換はまずサンドボックスで試行し、影響を要約してユーザ承認後に本番実行するフローを MCP で管理。
...
- 将来の拡張候補:
	...
	- セマンティック検索（自然文→空間クエリ）:
	  - ユーザの自然文クエリと現在の map_view/layers/selection を MCP に渡して、モデルが属性フィルタ＋空間条件（PostGIS クエリ候補）を生成。実行前プレビューと承認を必須とする運用。
	- 文脈ベースの結果要約・説明生成:
	  - 検索結果に対する要約文や利用上の注意を MCP 経由で自動生成し、非専門者向けの説明を付与。
	- 履歴を使ったコンテクスト検索:
	  - session_history を踏まえた連続的な検索支援（前回の検索・選択を参照して意図を補完）。
	- RAG（文脈＋ドキュメント検索）連携:
...
- 将来の拡張候補:
	...
	- スタイル転写支援（QGIS → MapLibre）:
	  - 現在の QGIS スタイルと公開要件（背景色、可読性基準）を MCP に渡し、モデルが MapLibre スタイル JSON の変換案・差分パッチを生成。
	- 公開向け説明文・ノート自動生成:
	  - パーマリンクやパッケージに添付する説明（目的・更新履歴・利用上の注意）を MCP 経由で生成して添付。
	- パッケージ化 / 簡易配布テンプレの提案:
...
- 将来の拡張候補:
	...
	- 出力説明文・キャプションの自動生成:
	  - PNG/PDF と出力メタ（テンプレ名・中心座標・スケール・レイヤ一覧）を MCP に渡し、モデルが図番、キャプション、脚注を自動生成して Excel に埋め込む。
	- 差分レポート（前回出力との比較）:
	  - 過去出力のコンテキストを参照し、変更点・注目点の自然文サマリを生成（会議資料向け）。
	- バッチ出力ワークフローの最適化提案:
	  - 大量 Atlas 出力の最適分割・並列化指示や、テンプレ適用順序の提案を MCP に基づいて生成。
	- 自動検査と承認ワークフロー:
	  - 自動生成された報告書候補に対して、チェックリスト生成と承認フローの案内を MCP が作成（人の確認を組み込む）。
